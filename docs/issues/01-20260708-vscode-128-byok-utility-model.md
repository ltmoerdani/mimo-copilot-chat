...
| Extension | `ltmoerdani.xiaomi-mimo-copilot-chat` — affected on any version ≤0.1.4 without this patch |
...
1. Set `chat.utilitySmallModel` to a specific MiMo model ID → requires knowing the VS Code-internal model ID format, fragile if the user changes their default model.
2. Set `chat.utilityModel` to a specific MiMo model ID → same problem.
...
Option 3 was chosen because it is model-agnostic: it works regardless of which MiMo model (mimo-v2.5, mimo-v2.5-pro, etc.) the user has selected.
...
**Effect:** VS Code routes all background utility tasks (title generation, commit messages, intent detection, rename suggestions) to the user's currently-selected MiMo BYOK model instead of failing.
...
This is the correct behavior for a BYOK-only extension like xiaomi-mimo-copilot-chat because:
- The user has already selected a MiMo model as their main agent.
...
Added after the `activate()` function. Called at the end of `activate()` after `warmModelPickerMetadata()`.
